 ...
 * Runs in O(nm), where n,m is the digit length.
 *
 * @param var1
 * @param var2
...
COMPUTE_STATUS do_div(longint_t *var1, const longint_t *var2)
{
    ...
    // normalize divisor so that the leading digit is >= 5
    ...
    if (leading < DIVISOR_PREFIX)
    {
        ...
        // we only normalize when it doesn't overflow
        COMPUTE_STATUS status = do_mult(var1, &mult);
        if (status != STATUS_OVERFLOW)
        {
            do_mult(&divisor, &mult);
        }
    }
    ...
    // shift through the entire var1 dividend
